...
namespace Example
{
    public class GetEmailsExample
    {
        public static void Main()
        {
            ...
            {
                // Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
                List<EmailPreview> result = apiInstance.GetEmails(inboxId, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit, limit, sortTheResultsByReceivedDateAndDirectionASCOrDESC, retryTimeout, delayTimeout, minCount, unreadOnly, before, since);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
