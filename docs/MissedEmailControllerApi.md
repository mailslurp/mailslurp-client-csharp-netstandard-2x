...
> MissedEmail WaitForNthMissedEmail (DateTime? before = null, Guid? inboxId = null, int? index = null, DateTime? since = null, long? timeout = null)
...
namespace Example
{
    public class WaitForNthMissedEmailExample
    {
        public static void Main()
        {
            ...
            {
                // Wait for Nth missed email
                MissedEmail result = apiInstance.WaitForNthMissedEmail(before, inboxId, index, since, timeout);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
